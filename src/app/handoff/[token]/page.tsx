    ...
    return (
      <div className="min-h-[70vh] flex items-center justify-center p-4">
        <div className="bg-canvas-paper border border-state-failure-border rounded-lg p-6 max-w-sm text-center space-y-2">
          ...
          <p className="text-sm text-ink-secondary">{error || "Handoff link has expired or is invalid."}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-md mx-auto py-4 px-2 space-y-5">
      ...
      <div className="text-center space-y-1">
        <span className="text-[13px] font-bold uppercase tracking-widest text-action-primary bg-action-primary/10 px-2.5 py-0.5 rounded-full inline-block">
      ...
      <div className="bg-canvas-paper border border-edge rounded-2xl p-6 shadow-sm space-y-5">
        <div className="flex items-center justify-between border-b border-edge pb-3 text-sm">
          <span className="text-[13px] font-semibold text-ink-muted uppercase">
            Receipt v{receipt.version}
          </span>
          {proofId && (
            <span className="font-mono text-sm px-2 py-0.5 rounded bg-blue-50 text-[#1B365D] border border-blue-200 font-bold">
              {proofId}
            </span>
          )}
        </div>
        ...
        <div className="text-center pb-4 border-b border-edge">
          <span className="text-sm uppercase font-bold text-ink-muted tracking-wider block">
        ...
        <div className="space-y-3 text-sm">
          <div className="flex justify-between items-start border-b border-edge/60 pb-3">
            <div>
              <span className="text-sm uppercase font-medium text-ink-muted block">Receiving Facility</span>
              <span className="font-bold text-ink-primary">{receipt.dock_name}</span>
            </div>
            {receipt.door && (
              <div className="text-right">
                <span className="text-sm uppercase font-medium text-ink-muted block">Designated Door</span>
        ...
        <div className="p-3 bg-canvas-subtle/60 rounded-xl border border-edge text-[13px] space-y-1">
          <div className="flex items-center justify-between">
            <span className="font-bold text-ink-primary uppercase tracking-wider text-sm">
        ...
        <p className="text-[13px] text-ink-muted text-center leading-tight">
