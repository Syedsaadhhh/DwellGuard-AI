    ...
    <div className="bg-canvas-paper border border-edge rounded-lg p-6 shadow-sm relative overflow-hidden space-y-5">
      ...
      <div className="flex items-center justify-between border-b border-edge pb-3">
        <div className="flex items-center space-x-2">
          <span className="text-sm uppercase font-bold tracking-wider text-ink-secondary">
            Confirmed Appointment Pass
          </span>
          <span className="text-[13px] px-2 py-0.5 rounded bg-state-confirmed-bg text-state-confirmed-text font-semibold border border-state-confirmed-border">
            Verified Agreement
          </span>
        </div>
        <div className="flex items-center space-x-2">
      ...
      <div className="flex flex-col sm:flex-row sm:items-baseline justify-between pt-1">
        <div>
          ...
          <p className="text-sm text-ink-muted mt-1">{dateFmt}</p>
        </div>

        <div className="mt-3 sm:mt-0 text-left sm:text-right">
          <span className="text-sm uppercase text-ink-muted font-medium block">Designated Dock</span>
          <span className="text-base font-bold text-ink-primary">{receipt.dock_name}</span>
          {receipt.door && (
            <span className="block text-sm font-semibold text-action-primary">
              {receipt.door}
            </span>
          )}
        </div>
      ...
      {/* Details Grid */}
      <div className="grid grid-cols-2 gap-3 pt-2 text-sm border-t border-edge/80">
        <div>
      ...
      <div className="flex items-center justify-between pt-1 text-sm text-ink-muted">
